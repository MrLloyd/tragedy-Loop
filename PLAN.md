...
- `PHASE7_GAP_CHECKLIST.md`：Phase 7 缺口清单，聚焦 FS / BTX 的规则、身份、事件、角色友好能力与特性补齐。
...
> 详细缺口、执行顺序与对表测试计划见 `PHASE7_GAP_CHECKLIST.md`。

#### P7-1 数据与范围收敛

- **目标**：先完成 `First Steps` / `Basic Tragedy X` 的模组规则能力与身份能力，再处理角色友好能力、角色特性与兼容字段清理。
- **规则盘点**：逐条核对 `规则Y` / `规则X` 的身份槽约束、失败条件、常驻能力、剧作家能力、轮回结束结算入口，明确哪些由 `script_validator` 负责，哪些由阶段/结算器负责。
- **身份盘点**：逐个核对 `data/modules/first_steps.json` 与 `data/modules/basic_tragedy_x.json` 中的身份定义，确认能力时机、条件、效果、限次、特性标签都已结构化。
- **事件盘点**：逐条核对 FS / BTX 事件定义，确认事件目标、事件发生条件、区域选择、原子结算入口与日志表现已接通。
- **角色盘点**：在模组规则与身份能力收敛后，再核对 `data/characters.json` 中的友好能力、角色特性、初始区域与信息公开边界。

#### P7-2 模组规则能力实现

- **身份槽规则**：补齐并测试 `fs_murder_plan`、`btx_murder_plan`、`btx_friends_circle`、`btx_love_scenic_line`、`btx_latent_serial_killer`、`btx_unknown_factor_chi` 等纯身份槽规则。
- **失败条件规则**：补齐并测试 FS / BTX 中所有 `loop_end` 失败条件，确认规则 Y / X 的失败判定只走一个明确入口。
- **常驻规则能力**：收敛 `btx_causal_line`、`btx_delusion_spread_virus` 等常驻或跨轮回规则的实现归属。
- **剧作家规则能力**：补齐 `fs_rumors` / `btx_rumors` 等剧作家能力的目标、时机与每轮限次测试。
- **Loop End 职责收敛**：明确 `LoopEndCheck` 与 `LoopEndHandler` 的分工，清理 `engine/phases/phase_base.py` 里的 TODO，避免失败条件分散实现。

#### P7-3 模组身份能力实现

- **关键身份能力**：优先补齐并验证 `传谣人`、`主谋`、`关键人物`、`亲友`、`不安定因子`、`魔女` 等对主流程影响最大的身份能力与特性。
- **动态身份能力**：明确 `不安定因子` 的派生能力来源，验证学校 2+ 密谋获得传谣人能力、都市 2+ 密谋获得关键人物能力。
- **身份触发链回归**：补最小可复现案例，覆盖 `亲友公开身份`、`不安定因子死亡链`、`心上人/求爱者死亡链`、`魔女无视友好` 等关键交互。
- **信息边界实现**：确认 `公开身份`、主人公侧可见快照、剧作家侧调试视图三者一致，避免只有状态位变化而 UI / 快照不可见。

#### P7-4 事件与角色结构化

- **事件结算回归**：补最小可复现案例，覆盖 `自杀` 成功结算、`失踪` 的目标区域选择与区域变更、以及事件触发后的日志与状态写入。
- **受控约定字符串**：保留 `any_board`、`choose_token_type`、`target=other` 等约定值，补合法值词表与 resolver 回归。
- **角色能力迁移**：完成 `data/characters.json` 里所有 `goodwill_ability_*` 旧三字段到 `goodwill_abilities` 的结构化迁移。
- **角色特性迁移**：在模组规则与身份能力稳定后，再拆 `trait_rule` 为结构化特性、能力或剧本制作约束。
- **兼容逻辑清理**：删除 `CharacterDef` / `CharacterState` / `AbilityResolver` / 静态校验中的旧字段 fallback 与正向支持，只保留结构化入口。

#### P7-5 测试矩阵与交付标准

- **逐条对表测试**：为 FS / BTX 每条 `规则Y` / `规则X` 至少补一条最小回归，区分身份槽约束、失败条件、常驻能力、剧作家能力四类。
- **角色能力测试**：为每个有友好能力或关键特性的角色至少补一条行为测试，不再只验证静态字段存在。
- **事件测试**：为每个已实现事件至少补一条“输入 → 原子结算 → 触发链 → 可见结果”的完整测试。
- **回归标准**：`engine.validation`、Phase 7 相关单测、关键 UI / controller 回归全部通过，且不再依赖旧友好能力字段。
- **文档同步**：更新 `PHASE7_GAP_CHECKLIST.md`、`WORK_PROGRESS.md` 与相关映射文档，记录已完成项、剩余缺口与测试入口。

#### P7-6 建议执行顺序

1. 先完成 FS / BTX 模组规则能力：身份槽、失败条件、常驻能力、剧作家能力。
2. 再完成 FS / BTX 模组身份能力：关键身份、动态身份、身份触发链、信息边界。
3. 然后补事件解释回归与受控约定字符串测试。
4. 最后处理角色友好能力、角色特性与旧字段清理。
5. 跑 Phase 7 全量测试矩阵并同步更新进度文档。
