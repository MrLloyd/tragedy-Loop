...
- `PLAN_CODE_MAPPING.md`：计划阶段 ↔ 代码路径索引。
- `PHASE7_GAP_CHECKLIST.md`：Phase 7 前四项测试收口与角色实现 / 验证清单。
- `docs/phase7_goodwill_structured_migration.md`：角色友好能力结构化迁移批次记录。
- `WORK_PROGRESS.md`：当前工作断点与下次继续建议。
...
- `data/characters.json` 当前共有 `37` 个角色。
- 当前只有 `女子学生`（`female_student`）、`男子学生`（`male_student`）、`偶像`（`idol`）、`职员`（`office_worker`）、`巫女`（`shrine_maiden`）的友好能力已真正落地。
- 已有部分角色侧基础可复用：
  - `AI`（`ai`）的事件判定特殊处理已存在。
  - `手下`（`henchman`）的“每轮初始区域选择”已接入。
  - `script_validator` 已接入部分“剧本制作时”角色约束。
- 剩余角色缺口主要集中在：`友好能力效果`、`角色特性`、`特殊登场 / 初始区域`、`事件联动`、`信息边界`。

#### P7-3 第一批：基础友好能力（按原语优先，先筛掉角色特性耦合）

- **目标**：先实现“只移动 / 只增改标记物 / 公开身份”的基础友好能力，但是否进入首批，先看该角色的**特性 / 生命周期 / 禁行 / 登场方式**会不会反向放大实现范围。
- **筛选规则**：
  - 只纳入**不依赖** `trait_rule`、`entry_loop`、`entry_day`、`initial_area_mode`、禁行区域解除、尸体/移除状态、`Ex` 牌、事件代打、跟随移动等机制的角色。
  - 同一角色若同时拥有“基础能力 + 复杂能力”，允许**按槽位部分落地**；但角色整体仍记为“未完成”。
  - 表面上是“只移动”的能力，如果实际会被禁行区域、角色特性或镜像区域规则改写，**不算首批低风险对象**。

##### P7-3a 只增改标记物（首批优先）

- **按“只增加/减少标记物，且没有角色属性限制”筛出的能力**：
  - `上位存在`（`higher_being`）友好1：同区域角色 `+1` 希望 / 绝望
  - `偶像`（`idol`）友好1/2：同区域其他角色 `-1` 不安 / `+1` 友好
  - `军人`（`soldier`）友好1：同区域角色 `+2` 不安
  - `刑警`（`detective`）友好2：同区域角色 `+1` 护卫
  - `医生`（`doctor`）友好1：同区域其他角色 `放置/移除 1` 不安
  - `大小姐`（`ojousama`）友好1：同区域角色 `+1` 友好（带区域条件）
  - `女子学生`（`female_student`）友好1：同区域其他角色 `-1` 不安
  - `媒体人`（`media_person`）友好1/2：其他角色 `+1` 不安；同区域角色/版图 `+1` 密谋
  - `巫女`（`shrine_maiden`）友好1：神社版图 `-1` 密谋（带区域条件）
  - `护士`（`nurse`）友好1：同区域达限其他角色 `-1` 不安
  - `教主`（`cult_leader`）友好1：任意达限角色 `+1` 友好
  - `男子学生`（`male_student`）友好1：同区域其他角色 `-1` 不安
  - `神灵`（`deity`）友好2：同区域角色或版图 `-1` 密谋
  - `转校生`（`transfer_student`）友好1：同区域其他角色 `-1` 密谋并 `+1` 友好
- **低风险首批**：
  - `偶像`（`idol`）友好1/2：同区域其他角色 `-1` 不安 / `+1` 友好
  - `军人`（`soldier`）友好1：同区域角色 `+2` 不安
  - `刑警`（`detective`）友好2：同区域角色 `+1` 护卫
  - `大小姐`（`ojousama`）友好1：同区域角色 `+1` 友好（带区域条件）
  - `女子学生`（`female_student`）友好1：同区域其他角色 `-1` 不安
  - `媒体人`（`media_person`）友好1/2：角色 `+1` 不安；角色/版图 `+1` 密谋
  - `巫女`（`shrine_maiden`）友好1：神社版图 `-1` 密谋（带区域条件）
  - `护士`（`nurse`）友好1：移除达限角色 `1` 不安
  - `男子学生`（`male_student`）友好1：同区域其他角色 `-1` 不安
  - `神灵`（`deity`）友好2：同区域角色或版图 `-1` 密谋
- **同原语但后移到第二批**：
  - `上位存在`（`higher_being`）友好1：需要“希望 / 绝望二选一”输入
  - `医生`（`doctor`）友好1：带“无视友好身份时剧作家可代用”的额外判定
  - `教主`（`cult_leader`）友好1：需要“不安达限度”条件稳定覆盖
  - `转校生`（`transfer_student`）友好1：需要“先移除再放置到同一条件目标”的两段式链路
  - `教师`（`teacher`）友好1：属于标记物原语，但带 `student` 属性限制，不纳入本轮“无角色属性限制”名单

##### P7-3b 公开身份（首批仅做低风险）

- **低风险首批**：
  - `教师`（`teacher`）友好2：公开同区域 `1` 名学生身份
- **同原语但后移到第二批**：
  - `大人物`（`vip`）友好1：`领地` 目标范围语义需先明确
  - `临时工？`（`temp_worker_alt`）友好1：公开自身身份，但与 `临时工` 的身份 / 当事人配置联动
  - `局外人`（`outsider`）友好1：带“第 2 轮后可用 + 不可拒绝”限制
  - `教主`（`cult_leader`）友好2：带“不安达限度”条件

##### P7-3c 只移动（首批先不抢开）

- **当前结论**：FS / BTX 范围内，表面是“只移动”的角色能力，当前都和角色侧规则存在耦合，不适合作为第一批低风险入口。
- **典型后移项**：
  - `小女孩`（`little_girl`）友好2：虽然是“移动至相邻版图”，但其基础禁行区域会直接影响可达区域
  - `幻想`（`phantom`）友好1：能力本身是移动，但角色特性会改写“版图牌同时作用于角色”的判定面
  - `医生`（`doctor`）友好2：本质是“解除禁行区域”，不属于纯移动原语

- **开工前只补最小公共能力，不再先铺大原语**：
  - 友好能力侧 `choose_token_type` 扩到 `hope / despair`
  - 条件回归：区域、属性、不安达限度、轮回数
  - `REVEAL_IDENTITY` 在“self / same_area / living target”路径的角色测试模板
  - `MOVE_CHARACTER` 与禁行 / 相邻 / `faraway` 的角色能力回归模板
- **验证要求**：优先按原语建测试批次（标记物 / 公开身份 / 移动），每落地 `1` 个角色至少补 `1` 条自动化测试。

#### P7-4 第二批：同原语但带角色特性 / 生命周期依赖
...
- **重点机制**：
  ...
  - 禁行区域解除与禁行回归
  - 延迟登场与角色已上场判定
- **说明**：这一批仍尽量复用 `MOVE_CHARACTER` / `PLACE_TOKEN` / `REMOVE_TOKEN` / `REVEAL_IDENTITY`，但要把角色特性与生命周期覆盖补齐后再落角色。

#### P7-5 第三批：新增原语或高交互 / 高风险角色
...
1. 前四项测试结论已在文档中固定，不再反复回滚。
2. 角色按四批推进：`简单能力 -> 生命周期 -> 高交互 -> 总测回归`。
3. 每批实现后立即补对应测试 / 调试验证，不累计到最后一起补。
4. `PHASE7_GAP_CHECKLIST.md`、`WORK_PROGRESS.md` 与实际角色实现进度同步更新。
