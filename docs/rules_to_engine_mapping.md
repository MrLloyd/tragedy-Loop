...
- 当前仓库中的 `data/modules/first_steps.json` 与 `data/modules/basic_tragedy_x.json` 已覆盖附录中的 `FS / BTX` 条目数量
- 当前主要缺口不在模组条目“有没有录入”，而在角色层是否完成结构化迁移
- 因此出现“规则似乎不完整”时，优先区分：
  1. 条目已录入，但行为仍依赖通用机制或角色层兼容数据
  2. UI / 适配层没有正确显示已录入内容
