...
以下语义已作为当前引擎的**明确结算不变量**，后续修改 `engine/resolvers/atomic_resolver.py`、`TurnEndHandler`、死亡触发条件/目标解析时，默认必须保持；若要改动，先补对应规则说明与回归测试。

1. **同一批次死亡必须完整触发全部死亡效果**
   - 当一次 `simultaneous` 结算中有多个角色同时死亡时，所有这些角色的 `on_death`、以及与之相关的 `on_other_death` 都必须进入触发收集。

2. **同一批次死亡的角色在触发死亡效果时，状态上仍按“已死亡”处理**
   - 同批次死亡只保证“触发要完整收集”，**不**额外回滚这些角色到“存活”视角。
   - 因此，死亡触发后续若依赖“是否存活”“是否可指定活体”“是否还能正常承受该效果”等语义，应按尸体状态解释。
   - 例如，同批次死亡的 `beloved / lover` 不应因为彼此同时死亡而再互相得到 6 枚 `[不安]`。

3. **先后死亡严格按批次分开**
   - 顺序结算中，前一批已经死亡的角色，不会在后一批死亡触发里被当作普通活体目标。
   - 只有规则明确写明可以指定尸体时，后续效果才应再次指向尸体。

4. **触发效果的 `sequential` 语义必须保留**
   - `sequential = false`：同一触发能力的效果作为一批同时规划。
   - `sequential = true`：按效果顺序逐步进入新的读-写-触发循环。

当前这组不变量至少由以下回归覆盖：
- `tests/test_phase1_core.py` 中 `beloved / lover` 同时死亡与先后死亡用例
