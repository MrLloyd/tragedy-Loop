...
`LOOP_END` 结算所有 `timing=loop_end` 能力后，`advance()` 在 `LOOP_END` 末尾进入 **`_branch_after_loop_end`**：
...
flowchart TB
    ...
    TE -->|非最终日| TS
    TE -->|最终日| LE[LOOP_END]
    LE -->|未失败| GE[GAME_END 胜]
    LE -->|失败且非最后轮回| NL[NEXT_LOOP]
    NL --> RFL[reset_for_new_loop]
    RFL --> LS[LOOP_START]
    LS --> TS
    LE -->|失败且最后轮回| FG[FINAL_GUESS 或 GAME_END]
```
...
- `ACTION_RESOLVE` / `INCIDENT` / `TURN_END` 等业务规则未完成。
