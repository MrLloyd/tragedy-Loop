...
- [ ] 行动牌 / 区域投影 / 跟随与代死
  ...
  - `phantom`：行动牌路由已接入；`NO_ACTION_CARDS` 直接禁止在该角色身上放牌，版图牌按移动批次与非移动批次分别投影到当前所在角色
  - `servant`：跟随 `vip / ojousama` 移动已接入，支持同批次多目标由主人公选择、忽略自身同阶段移动、并覆盖 `sequential`/“随后”；`trait_target_overrides` 已预留给友好能力扩展。仍缺代替死亡链
