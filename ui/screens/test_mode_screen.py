...
try:  # pragma: no cover - optional UI dependency
    ...
    class TestModeScreen(QWidget):
        ...
        def __init__(
            ...
            self.script_incidents_widget = QWidget()
            script_incidents_layout = QVBoxLayout(self.script_incidents_widget)
            script_incidents_layout.setContentsMargins(0, 0, 0, 0)
            script_incidents_layout.addWidget(QLabel("事件日程"))
            self.script_incidents_grid = QGridLayout()
            script_incidents_layout.addLayout(self.script_incidents_grid)
            rules_form.addRow("规则 Y", self.rule_y_input)
            rules_form.addRow("规则 X", self.rule_x_widget)
            rules_form.addRow("", self.script_incidents_widget)
            ...
            phase_actions.addWidget(self.enter_goodwill_phase_button)
            ...
            character_actions.addWidget(self.run_formal_flow_button)
